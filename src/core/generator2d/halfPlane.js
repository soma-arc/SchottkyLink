...
export default class HalfPlane extends Generator {
    ...
    move(selectionState, mouse) {
        if (selectionState.componentId === HalfPlane.BODY ||
            selectionState.componentId === HalfPlane.ORIGIN_POINT) {
    ...
    exportAsQueryString() {
        return `HalfPlane[]=${this.p.x.toFixed(this.digits)},${this.p.y.toFixed(this.digits)},${this.normalAngleDeg.toFixed(this.digits)}`;
    }
