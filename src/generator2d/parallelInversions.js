...
export default class ParallelInversions extends Generator {
    /**
    ...
    select(mouse, sceneScale) {
        ...
        if (dpNormal.length() < this.UIPointRadius * sceneScale) {
            return new SelectionState().setObj(this)
                .setComponentId(ParallelInversions.NORMAL_POINT)
                .setDiffObj(dpNormal);
        }
    ...
    selectBody(mouse, sceneScale) {
        ...
        if (dp2.length() < this.UIPointRadius * sceneScale) {
            return new SelectionState().setObj(this)
                .setComponentId(ParallelInversions.POINT_HP2)
                .setDiffObj(dp2);
        }
        ...
        return new SelectionState().setObj(this)
            .setComponentId(ParallelInversions.BODY)
            .setDiffObj(dp);
    }

    isHandle(componentId) {
        return componentId === ParallelInversions.NORMAL_POINT ||
            componentId === ParallelInversions.POINT_HP2;
    }
    ...
    move(mouseState, mouse) {
        ...
        case ParallelInversions.NORMAL_POINT: {
        ...
        case ParallelInversions.POINT_HP2: {
    ...
    setUniformLocation(gl, uniLocation, program, index) {
        uniLocation.push(gl.getUniformLocation(program,
                                               `u_parallelInversions${index}.p`));
        uniLocation.push(gl.getUniformLocation(program,
                                               `u_parallelInversions${index}.normal`));
        uniLocation.push(gl.getUniformLocation(program,
                                               `u_parallelInversions${index}.ui`));
        uniLocation.push(gl.getUniformLocation(program,
                                               `u_parallelInversions${index}.selected`));
    }

    exportAsQueryString() {
        return `ParallelInversions[]=${this.p.x.toFixed(this.digits)},${this.p.y.toFixed(this.digits)},${this.normalAngleDeg.toFixed(this.digits)},${this.planeDist.toFixed(this.digits)}`;
    }
